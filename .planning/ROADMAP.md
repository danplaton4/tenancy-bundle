...
- [ ] **Phase 27: Async Shared Entities** — opt-in `tenancy.shared.async: true` mode with Messenger fan-out via `SharedEntityChangedMessage` + AsyncCanaryTest pattern (SHARE-03)
  ...
  - Plans:
    - [ ] 27-01-PLAN.md — Wave 1: contract layer — `tenancy.shared.async` boolean config node + unconditional parameter (D-06), `SharedEntityChangedMessage` scalar value object (SHARE-03-c), `SharedEntityAsyncFanOutException extends \RuntimeException` (D-02), `SharedAsyncContractPass` compile-time guard (SHARE-03-i) + Wave 0 unit tests
