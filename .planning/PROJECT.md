...
---
*Last updated: 2026-06-11 — v0.4 Storage & Shared Entities in progress: Phase 25 (SHARE-01 Shared Entities, Sync mode) complete — `#[Shared]` marker attribute, `SharedEntitySyncSubscriber` (onFlush-buffer → postFlush best-effort fan-out, scalar-only copy, shared_db no-op, re-entrancy bypass, tenant-context save/restore + connection reset around the fan-out), `SharedEntityWriteProtectionListener` (tenant-side read-only guard), `SharedEntityMutualExclusionPass` (compile-time `#[Shared]`+`#[TenantAware]` guard), `SharedEntityWriteInTenantContextException`. Code review caught + fixed 3 Critical tenant-isolation findings before completion. 706 PHPUnit tests / 3003 assertions, PHPStan level 9 clean, cs-fixer clean. Phases 26–29 (shared resync command, async shared entities, PHPStan extension) remain.*
