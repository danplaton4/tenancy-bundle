...
---
*Last updated: 2026-06-15 — v0.4 Storage & Shared Entities in progress: Phase 26 (SHARE-02 `tenancy:shared:resync` command) complete — bulk drift-repair console command mirroring `tenancy:migrate` (`--tenant`/`--dry-run`/`--force`), built on a `SharedEntityCopier` extracted from the Phase 25 subscriber (shared `applyRow`/`classifyRow`/`findSharedClasses`/`isShared` + the `syncInProgress` re-entrancy flag, now consulted by `SharedEntityWriteProtectionListener`). Two-pass classify→confirm→apply, continue-on-failure per-tenant, `shared_db` informational no-op. Code review caught + fixed 1 Critical (CR-01: apply pass now resets the tenant EM after a failed flush, mirroring the subscriber — restores D-06 isolation) plus honest classify-failure reporting; advisory WR-04/WR-05/IN-01..03 deferred. Verification 12/12 must-haves; 1 live-TTY confirm-prompt check pending in 26-HUMAN-UAT.md. 730 PHPUnit tests / 3130 assertions, PHPStan level 9 clean, cs-fixer clean. Phase 25 (SHARE-01 Sync mode) shipped prior. Phases 27–29 (async shared entities, PHPStan extension, docs) remain.*
