...
```bash
UI_AUDITOR_MODEL=$(node "/Users/danplaton/dev/packages/[claude-code] symfony-multitenancy/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-ui-auditor --raw)
```
...
```markdown
Read /Users/danplaton/dev/packages/[claude-code] symfony-multitenancy/.claude/agents/gsd-ui-auditor.md for instructions.
...
```bash
node "/Users/danplaton/dev/packages/[claude-code] symfony-multitenancy/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): UI audit review" --files "${PHASE_DIR}/${PADDED_PHASE}-UI-REVIEW.md"
```
