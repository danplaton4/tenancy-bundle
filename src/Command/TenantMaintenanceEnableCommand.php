...
/**
 * Puts a single tenant into maintenance mode.
 *
 ...
 * cache.app to ensure the maintenance state takes effect immediately (RESEARCH §Cache Coherence).
 ...
 *
 * Operation is idempotent (D-08): a second enable on an already-in-maintenance tenant exits
 * SUCCESS without a flush, cache delete, or event dispatch.
 */
#[AsCommand(
    name: 'tenancy:maintenance:enable',
    description: 'Put a tenant into maintenance mode (idempotent)',
)]
final class TenantMaintenanceEnableCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // Idempotent guard (D-08): already in maintenance → print message, exit SUCCESS.
        ...
        $this->eventDispatcher->dispatch(new TenantMaintenanceEnabled($tenant));

        $io->success(sprintf('Tenant "%s" is now in maintenance mode.', $slug));
